...
info_sites_temp_ARCHIVE <- search_database("archive", "temp", extra = TRUE)    ### sites where temperature is measured
...
write.csv(Archive_temp_sites, "Archive_temp_sites.csv")
...
popup_temp <- paste("<p><strong>", temperature_data_ARCHIVE$site_name,":", "</strong>", sep = " ",
                     round(temperature_data_ARCHIVE$temp), " C")
...
# META_UK <- read.csv("META_10Dec2014_PM10.csv")
# Random <- getMeta(lat = 51.35220, lon = -1.990416)
...
#  for (i in 121:121)  {
  ...
  temp <- temp %>%
     mutate(date = ymd_hms(date, tz = "UTC"),
            day = date(date))
  temp <- filter(temp, day == "2016-05-22") 
...
write.csv(temp_UK, "temp_UK_2016_05_08.csv")
